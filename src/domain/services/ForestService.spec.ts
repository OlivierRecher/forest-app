...
describe('ForestService', () => {
    ...
    describe('list', () => {
        it('should return all forests', () => {
            const forests: Forest[] = [{ id: '1', type: ForestType.BOREAL, surface: 100 }];
    ...
    describe('update', () => {
        ...
        it('should throw error on update with invalid data', () => {
            ...
            expect(() => forestService.update('1', forest)).toThrow('Invalid forest type');
        });

        it('should throw error on update with invalid surface', () => {
            const forest: Forest = { id: '1', type: ForestType.BOREAL, surface: 0 };
            expect(() => forestService.update('1', forest)).toThrow('Invalid surface area');
        });
    });

    describe('addTree', () => {
        it('should add a tree to the forest', () => {
            const existingForest: Forest = { id: '1', type: ForestType.BOREAL, surface: 100, trees: [] };
            repoMock.findAll.mockReturnValue([existingForest]);

            const tree: Tree = { id: 't1', birth: new Date(), species: Species.OAK, exposure: Exposure.SUNNY, carbonStorageCapacity: 10 };
    ...
    describe('calculateSurfaceNeeded', () => {
        ...
        it('should throw error if current absorption is 0', () => {
            ...
            expect(() => forestService.calculateSurfaceNeeded('1', 1000)).toThrow('This forest has no CO2 absorption capacity');
        });

        it('should throw NotFoundError if forest not found', () => {
            repoMock.findAll.mockReturnValue([]);
            expect(() => forestService.calculateSurfaceNeeded('999', 1000)).toThrow(NotFoundError);
        });
    });

    describe('getSpecies', () => {
        it('should return empty list if forest has no trees', () => {
            const forest: Forest = { id: '1', type: ForestType.BOREAL, surface: 100, trees: [] };
            repoMock.findAll.mockReturnValue([forest]);
        ...
        it('should return unique species list', () => {
            ...
            const tree2: Tree = { id: 't2', birth: new Date(), species: Species.OAK, exposure: Exposure.SUNNY, carbonStorageCapacity: 10 };
            const tree3: Tree = { id: 't3', birth: new Date(), species: Species.ASH, exposure: Exposure.SUNNY, carbonStorageCapacity: 10 };
